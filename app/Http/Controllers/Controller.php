...
class Controller extends BaseController
{
    ...
    public function processCsvV1(Request $request, $csvFile='')
    {
        ...
        $csvFile =  public_path('examples-284-29-1-.csv');// $request->csvFile;
        ...
        while( $data = fgetcsv($file)) {
            ...
            foreach ($data as $name) {
                ...
                if(str_contains(strtolower(trim($name)), ' and ')){
                    ...
                    if (count($parts) > 1  ) {
                        if(str_contains(strtolower(trim($name)), 'mr and') || str_contains(strtolower(trim($name)), 'mrs and') ){
                            $parts[0] = $parts[0].' ' .substr(end($parts),  strpos(end($parts), ' '));
                        }
                    }
                }
                ...
                if(str_contains(strtolower(trim($name)), ' & ')){
                    $parts = explode(' & ', $name);
                    if (count($parts) > 1) {
                        if(str_contains(strtolower(trim($name)), ' & ') || str_contains(strtolower(trim($name)), ' & ') ){
                            $parts[0] = $parts[0].' ' .substr(end($parts),  strpos(end($parts), ' '));
                        }
                    }
                }
                ...
                foreach ($parts as $part) {
                    // Split each part into title, first name, initial, and last name
                    $person = [
                        'title' => null,
                        'first_name' => null,
                        'initial' => null,
                    ...
                    $namePosition = strtok(trim($part), ' ');
                    if (in_array($namePosition, $this->titleArray)) {
                        $person['title'] = $namePosition;
                        $namePosition = strtok(" ");
                    }
                    ...
                    if(strlen($namePosition) >= 3 && strrpos(trim($part), ' ') !== false){
                        $isLastname = explode(' ', $part);
                        if(strtolower( trim(array_pop($isLastname)) ) !== strtolower(trim($namePosition)) ) {
                            $person['first_name'] = $namePosition;
                        }
        ...
        // return response()->json($people);
        return view('index', compact('people')); // redirect()->route('/')->with($people);
